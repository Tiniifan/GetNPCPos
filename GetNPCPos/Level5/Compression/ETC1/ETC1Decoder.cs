...
namespace GetNPCPos.Level5.Compression.ETC1
{
    public static class ETC1Decoder
    {
        ...
        public static byte[] DecompressETC1A4(byte[] data, int width, int height, bool hasAlphaCanal)
        {
            byte[] result;
            ...
            byte[] alphaBlockData = hasAlphaCanal ? new byte[8] : null;
            byte[] blockData = new byte[8];
            byte[] alphas = hasAlphaCanal ? new byte[16] : null;
            byte[] colors = new byte[48]; // 16 * 3
            ...
            for (int blockY = 0; blockY < height; blockY += 4)
            {
                for (int blockX = 0; blockX < width; blockX += 4)
                {
                    if (hasAlphaCanal)
                    {
                        Array.Copy(data, offset, alphaBlockData, 0, 8);
                        offset += 8;
                        alphas = DecodeBlockAlphas(alphaBlockData);
                    }

                    Array.Copy(data, offset, blockData, 0, 8);
                    offset += 8;
                    colors = DecodeBlockColors(blockData);

...
using GetNPCPos.Level5.Binary.Logic;

namespace GetNPCPos.Level5.Binary
{
    public class CfgBin
    {
        ...
        public List<int> GetPtreeValue(string entryName)
        {
            // Use LINQ to search for the entry with the specified name
            var entry = Entries
                .SelectMany(e => FindPtreeRecursive(e, entryName))
                .FirstOrDefault();

            // If the entry is found, extract and return values in int format
            if (entry != null)
            {
                return entry.Children.Take(4)
                    .SelectMany(child => child.Variables)
                    .Where(variable => variable.Value is float || variable.Value is int)
                    .Select(variable => Convert.ToInt32(variable.Value))
                    .ToList();
            }

            // If the entry is not found, return an empty list
            return new List<int>();
        }

        public IEnumerable<Entry> FindPtreeRecursive(Entry entry, string entryName)
        {
            if (entry.Variables.Count != 0 && entry.Variables[0].Value is OffsetTextPair)
            {
                OffsetTextPair offsetTextPair = (OffsetTextPair)entry.Variables[0].Value;

                if (offsetTextPair.Text == entryName)
                {
            ...
            {
                foreach (var childEntry in entry.Children.SelectMany(e => FindPtreeRecursive(e, entryName)))
                {
